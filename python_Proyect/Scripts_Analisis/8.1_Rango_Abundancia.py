...
# --- 8. Exportar a Excel ---
ruta_salida_xlsx = r"D:\CORPONOR 2025\Backet\python_Proyect\Resultados\8.2_Curvas_Rango_Abundancia.xlsx"
...
# --- Rutas ---
ruta_original = r"D:\CORPONOR 2025\Backet\python_Proyect\Resultados\8.2_Curvas_Rango_Abundancia.xlsx"
ruta_limpia = r"D:\CORPONOR 2025\Backet\python_Proyect\Resultados\8.2_Curvas_Rango_Abundancia.xlsx"
...
def plot_curvas_comparativas_estilo(curvas_df, figsize=(12, 7)):
    ...
    import seaborn as sns
    colores = sns.color_palette("Paired", 12)
    abundancia_por_cobertura = (
        curvas_df.groupby("COBERTURA")["INDIVIDUOS"].sum().sort_values(ascending=False)
    )
...
#----------------------------------- GUARDAR GRÁFICO -----------------------------------
    ...
    output_folder = r"D:\CORPONOR 2025\Backet\python_Proyect\Resultados"
    os.makedirs(output_folder, exist_ok=True)
    ruta_salida = os.path.join(output_folder, "8.2.1_Curvas_Rango_Abundancia_Combinado.png")
...
#------------------- Curvas rango–abundancia comparativas (Minimalista Profesional – Excel Friendly) --------------------
...
def plot_curvas_comparativas_estilo(curvas_df, figsize=(12, 7)):
    ...
    # Colores profesionales, suaves, no saturados
    colores = [
        (0.15, 0.35, 0.55),  # azul petróleo
        (0.65, 0.25, 0.25),  # rojo suave
        (0.25, 0.55, 0.25),  # verde bosque
        (0.55, 0.45, 0.20),  # ocre
        (0.35, 0.25, 0.55),  # púrpura tenue
        (0.25, 0.45, 0.55),  # azul grisáceo
        (0.45, 0.45, 0.45),  # gris profesional
        (0.20, 0.20, 0.20)   # grafito
    ]

    abundancia_por_cobertura = (
        curvas_df.groupby("COBERTURA")["INDIVIDUOS"]
        .sum()
        .sort_values(ascending=False)
    )
    ...
    for i, (cobertura, _) in enumerate(abundancia_por_cobertura.items()):
        ...
        # ==========================================================
        #   Etiquetas (5 especies representativas)
        # ==========================================================
        n_especies = len(sub)
        indices = [0]  # siempre la más abundante

        if n_especies > 5:
            indices += list(np.linspace(1, n_especies - 1, 4, dtype=int))
        else:
            indices = list(range(n_especies))

        for idx in indices:
            especie = abreviar_nombre(sub.loc[idx, "ESPECIE"])
            x = sub.loc[idx, "RANGO"]
            y = sub.loc[idx, "LOG10_ABUND"]

            txt = ax.text(
                x, y, especie,
                fontsize=9, ha='center', va='bottom',
                style='italic', rotation=30, color='black'
            )
            textos.append(txt)

        # Control del desplazamiento horizontal
        factor_desplazamiento = 1
        offset += len(sub) * factor_desplazamiento

    # ==========================================================
    #   Ajuste automático de etiquetas (sin solapamientos)
    # ==========================================================
    adjust_text(
        textos, ax=ax,
        arrowprops=dict(arrowstyle="-", lw=0.6, color="gray", alpha=0.5),
        expand_points=(1.15, 1.15), force_text=0.65
    )
    ...
    # ==========================================================
    ax.set_xlabel("Rango de especies", fontsize=12)
    ax.set_ylabel("Log₁₀ (Abundancia + 1)", fontsize=12)
    ...
    output_folder = r"D:\CORPONOR 2025\Backet\python_Proyect\Resultados"
    os.makedirs(output_folder, exist_ok=True)
    ruta_salida = os.path.join(output_folder, "8.2.1_Curvas_Rango_Abundancia_CombinadoPRO.png")
...
def plot_curvas_por_cobertura_estilo(curvas_df, figsize=(8, 6)):
    ...
    for i, (cobertura, _) in enumerate(abundancia_por_cobertura.items()):
        ...
        nombre_archivo = f"8.2.1_Curva_Rango_Abundancia_{cobertura.replace(' ', '_')}.png"
...
def plot_curvas_grid_estilo(curvas_df, ncols=3, figsize_per_col=(5, 6)):
    ...
    # --- Guardar figura ---
    nombre_archivo = "8.2.2_Curvas_rango_abundancia_por_cobertura_grid.png"
    ruta_salida = os.path.join(output_folder, nombre_archivo)
    fig.savefig(ruta_salida, dpi=300, bbox_inches="tight")
...
def interpretar_rango_abundancia(curvas_df):
    texto = []
    # -------------------------------------------------------------------
    # --- 1. INTERPRETACIÓN GENERAL ---
    # -------------------------------------------------------------------
    
    total_especies = curvas_df["ESPECIE"].nunique()
    total_individuos = curvas_df["INDIVIDUOS"].sum()

    # Especies más abundantes (top 5)
    top_especies = curvas_df.sort_values("INDIVIDUOS", ascending=False).head(5)

    texto.append("=== INTERPRETACIÓN GENERAL DEL RANGO–ABUNDANCIA ===\n")
    texto.append(
        f"El ensamblaje total está compuesto por {total_especies} especies "
        f"y un total de {total_individuos} individuos registrados. "
        "La curva rango–abundancia evidencia el grado de dominancia biológica "
        "y la distribución de las abundancias entre especies.\n"
    )

    texto.append("Las especies más dominantes del paisaje general son:\n")
    for _, row in top_especies.iterrows():
        texto.append(f" • {row['ESPECIE']} ({row['INDIVIDUOS']} individuos)")

    texto.append("\nEstas especies determinan la pendiente inicial de la curva, indicando un patrón de dominancia pronunciado.\n")

    # -------------------------------------------------------------------
    # --- 2. CLASIFICACIÓN AUTOMÁTICA POR NIVELES DE ABUNDANCIA ---
    # -------------------------------------------------------------------
    texto.append("\n=== CLASIFICACIÓN DE ESPECIES SEGÚN SU ABUNDANCIA ===\n")

    q1 = curvas_df["INDIVIDUOS"].quantile(0.25)
    q3 = curvas_df["INDIVIDUOS"].quantile(0.75)

    altas = curvas_df[curvas_df["INDIVIDUOS"] >= q3]
    intermedias = curvas_df[(curvas_df["INDIVIDUOS"] < q3) & (curvas_df["INDIVIDUOS"] > q1)]
    bajas = curvas_df[curvas_df["INDIVIDUOS"] <= q1]

    texto.append(
        f"• Especies de **alta abundancia** (≥ {int(q3)} individuos): {altas['ESPECIE'].nunique()} especies.\n"
        f"• Especies de **abundancia intermedia**: {intermedias['ESPECIE'].nunique()} especies.\n"
        f"• Especies de **baja abundancia** (≤ {int(q1)} individuos): {bajas['ESPECIE'].nunique()} especies.\n"
    )

    texto.append(
        "Este patrón sugiere un ensamblaje con pocas especies dominantes y un número "
        "considerable de especies raras, lo cual es típico en ecosistemas diversos.\n"
    )

    # -------------------------------------------------------------------
    # --- 3. INTERPRETACIÓN POR COBERTURA ---
    # -------------------------------------------------------------------
    texto.append("\n=== INTERPRETACIÓN POR COBERTURA ===\n")

    coberturas = curvas_df["COBERTURA"].unique()

    for cob in coberturas:
        df_c = curvas_df[curvas_df["COBERTURA"] == cob]

        n_sp = df_c["ESPECIE"].nunique()
        n_ind = df_c["INDIVIDUOS"].sum()
        top_local = df_c.sort_values("INDIVIDUOS", ascending=False).head(3)

        texto.append(f"\n--- Cobertura: {cob} ---\n")
        texto.append(
            f"Esta cobertura contiene {n_sp} especies y un total de {n_ind} individuos. "
            f"La estructura de abundancias muestra una dominancia marcada por las siguientes especies:\n"
        )
        for _, row in top_local.iterrows():
            texto.append(f" • {row['ESPECIE']} ({row['INDIVIDUOS']} individuos)")

        # Clasificación local
        q1c = df_c["INDIVIDUOS"].quantile(0.25)
        q3c = df_c["INDIVIDUOS"].quantile(0.75)

        altas_c = df_c[df_c["INDIVIDUOS"] >= q3c]["ESPECIE"].nunique()
        bajas_c  = df_c[df_c["INDIVIDUOS"] <= q1c]["ESPECIE"].nunique()

        texto.append(
            f"\nLa cobertura presenta {altas_c} especies de alta abundancia "
            f"y {bajas_c} especies raras, lo que permite inferir su estado ecológico "
            f"y el grado de heterogeneidad en la disponibilidad de recursos.\n"
        )

    # -------------------------------------------------------------------
    # --- 4. SÍNTESIS FINAL ---
    # -------------------------------------------------------------------
    texto.append("\n=== SÍNTESIS FINAL DEL ENSAMBLAJE ===\n")
    texto.append(
        "El análisis de curvas de rango–abundancia revela un ensamblaje estructurado "
        "por unas pocas especies dominantes que concentran la mayor parte de los individuos, "
        "acompañadas por un amplio conjunto de especies raras de baja abundancia. "
        "Este patrón indica un ecosistema diverso, con gradientes ambientales que permiten "
        "la coexistencia de especies con distintos grados de especialización. "
        "Las diferencias entre coberturas sugieren variaciones en estructura del hábitat, "
        "recursos disponibles y perturbaciones locales que moldean la composición de cada zona."
    )
...
interpretacion = interpretar_rango_abundancia(curvas_df)
...
# Generar interpretación automáticamente
interpretacion = interpretar_rango_abundancia(curvas_df)

# Ruta final del archivo
ruta_txt = os.path.join(ruta_resultados, "8.2.3_Interpretacion_Rango_Abundancia.txt")

# Guardar en TXT
with open(ruta_txt, "w", encoding="utf-8") as f:
    f.write(interpretacion)

print(f"\n Interpretación guardada exitosamente en:\n {ruta_txt}")



#------------------- Curvas rango–abundancia por cobertura (en grilla, estilo publicación profesional) --------------------
import matplotlib.pyplot as plt
import numpy as np
from adjustText import adjust_text
import os

# ---- Función abreviar nombres científicos ----
def abreviar_nombre(nombre):
    partes = nombre.split()
    if len(partes) >= 2:
        return f"{partes[0][0]}. {partes[1]}"
...
def plot_curvas_grid_estilo(curvas_df, ncols=3, figsize_per_col=(5, 6)):
    ...
    colores = [(0.15, 0.35, 0.55), (0.65, 0.25, 0.25), (0.25, 0.55, 0.25),
               (0.55, 0.45, 0.20), (0.45, 0.25, 0.55), (0.25, 0.45, 0.55),
               (0.45, 0.45, 0.45), (0.20, 0.20, 0.20)]

    # --- Ordenar coberturas ---
    abund_por_cob = curvas_df.groupby("COBERTURA")["INDIVIDUOS"].sum().sort_values(ascending=False)
    orden_cob = ["TOTAL"] + [c for c in abund_por_cob.index if c != "TOTAL"]

    # --- Límites globales del eje Y ---
    ymin = curvas_df["LOG10_ABUND"].min() - 0.2
    ymax = curvas_df["LOG10_ABUND"].max() + 0.2
    ...
    fig_w = figsize_per_col[0] * ncols
    fig_h = figsize_per_col[1] * nrows + 1.2

    fig, axes = plt.subplots(nrows, ncols, figsize=(fig_w, fig_h), sharey=True)
    axes = np.atleast_2d(axes)

    # =====================================================
    #   Iteración por cada cobertura
    # =====================================================
    idx = 0
    for r in range(nrows):
        for c in range(ncols):
            ...
            if not sub.empty:
                ...
                # — Etiquetas de especies representativas —
                textos = []
                n_especies = len(sub)

                indices = [0]  # la más abundante siempre
                if n_especies > 5:
                    indices += list(np.linspace(1, n_especies - 1, 4, dtype=int))
                else:
                    indices = list(range(n_especies))
                ...
                for j in indices:
                    especie = abreviar_nombre(sub.loc[j, "ESPECIE"])
                    x = sub.loc[j, "RANGO"]
                    y = sub.loc[j, "LOG10_ABUND"]
                    txt = ax.text(
                        x, y, especie,
                        fontsize=8, ha="center", va="bottom",
                        rotation=40, style="italic"
                    )
                    textos.append(txt)
                ...
                adjust_text(
                    ...
                    expand_points=(1.15, 1.15), force_text=0.7
                )

            # -----------------------------------------------------
            # Ajustes estéticos del panel
            # -----------------------------------------------------
            ax.set_title(cov, fontsize=12, fontweight="bold", pad=8)
    ...
    # =====================================================
    plt.suptitle(
        "Curvas rango–abundancia por cobertura",
